...
export const createRequest = asyncHandler(async (req, res, next) => {
  ...
  // FIX 1: Only block if status is Pending or Connected, ignore Rejected
  ...
  // FIX 2: Delete any old Rejected record before creating new one
...
export const acceptRequest = asyncHandler(async (req, res, next) => {
  ...
  await Request.findOneAndUpdate({ sender: requestId, receiver: senderId }, { status: "Connected" });
...
export const rejectRequest = asyncHandler(async (req, res, next) => {
  ...
  // FIX 3: Delete the record instead of keeping it as Rejected
  await Request.findOneAndDelete({ sender: requestId, receiver: senderId });
